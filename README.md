...
- **Inside the decisions** expands into a full-width diagnostics panel. Expand individual moves to see all legal-move probabilities, confidence, API latency, browser round-trip time, prior visits, and history. Nested disclosures show the exact model request (discovered state, instructions, criteria) and the returned answer with token usage. Run totals cover returned moves; missing usage is shown as `—`. Failed and stopped requests may incur usage not included in these totals. Confidence describes the distribution's concentration, not the chance of reaching the goal. BFS shows its own path without AI metrics.
